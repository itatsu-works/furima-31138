...
RSpec.describe Item, type: :model do
  ...
  describe '商品登録' do
    context '商品登録がうまくいく時' do
      it '販売価格、発送までの日数、発送元の地域、配送料の負担、商品の状態、カテゴリー、商品の説明、商品名が存在する時、登録できる' do
    ...
    context '商品登録がうまくいかない時' do
      ...
      it '配送料の負担が空だと登録できない' do
        @item.cost_burden_id = ''
        @item.valid?
        expect(@item.errors.full_messages).to include("Cost burden can't be blank", 'Cost burden is not a number')
      end
      ...
      it '販売価格が300円より小さいと登録できない' do
        @item.price = '299'
        @item.valid?
        expect(@item.errors.full_messages).to include('Price must be greater than or equal to 300')
      end
      it '販売価格が9999999円より大きいと登録できない' do
        @item.price = '10000000'
        @item.valid?
